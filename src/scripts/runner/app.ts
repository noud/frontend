...
export const common = {
  ...
  websocketPort: (process.env.WS_PORT && parseInt(process.env.WS_PORT)) || undefined,
...
// Dev server
export async function devServer(koaApp: Koa, compiler: webpack.MultiCompiler, opt: PartialAll<KoaWebpack.Options> = {}) {
  ...
  const defaultOptions: KoaWebpack.Options = {
    ...
    devMiddleware: {
      ...
      stats: false,
      watchOptions: {
        aggregateTimeout: process.env.REBUILD_TIMEOUT ? Number(process.env.REBUILD_TIMEOUT) : 1000,
      },
    },
  ...
  const options = mergeWith(defaultOptions, opt);

  // Create the middlware, by merging in any overrides
  const koaWebpackMiddleware = await KoaWebpack(options);
...
if (common.isProduction) {
  app.use(staticMiddleware(path.resolve(dist, 'public')));
}

// ... and then fall-back to <root>/src/public
app.use(staticMiddleware(path.resolve(root, 'src', 'public'), false));
